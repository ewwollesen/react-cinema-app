...
const Header = (props) => {
  ...
  useEffect(() => {
    ...
    if (routesArray.length) {
      if (!path && !url) {
        pathURL('/', '/');
        const error = new Error(`Page with pathname ${location.pathname} not found with status code of 404.`);
        setError({ message: `Page with pathname ${location.pathname} not found with status code of 404.`, statusCode: 404 });
        throw error;
      }
    }
  ...
  useEffect(() => {
    if (errors.message || errors.statusCode) {
      pathURL('/', '/');
      const error = new Error(`${errors.message} with status code of ${errors.statusCode}.`);
      setError({ message: `Page with pathname ${location.pathname} not found with status code of 404.`, statusCode: 404 });
      throw error;
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
